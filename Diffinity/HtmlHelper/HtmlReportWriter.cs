...
<body>
    ...
    <table>
        <tr>
            ...
            <th>{MetaData} Name</th>
            <th>
                <label class=""hdr""><input type=""checkbox"" id=""chk-src-all""><span>{source} Original</span></label>
            </th>
            <th>
                <label class=""hdr""><input type=""checkbox"" id=""chk-dst-all""><span>{destination} Original</span></label>
            </th>
            {differences}
        </tr>
    ";
...
<head>
    ...
    public static (string html, string countObjects) WriteSummaryReport(DbServer sourceServer, DbServer destinationServer, string summaryPath, List<dbObjectResult> results, DbObjectFilter filter, Run run, bool isIgnoredEmpty,string ignoredCount)
    {
        ...
        string copySection = existingObjects.Any()
